...
| 3 | **Tool-allow-list, orchestrator roept aan** | `AskOrchestrator` houdt een dictionary corpus → `ISearchTool`; het model krijgt geen tool-definities | Tool-keuze, tool-argumenten; categorie uit de intent is een voorkeur met retry zonder categorie bij zwakke hits | unit-tests `AskOrchestratorTests` |
