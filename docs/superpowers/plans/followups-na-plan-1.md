...
| 22 | g04 ("wat is de ZoetermeerPas") → `escalated (no_cited_answer)` in runs 33167786538 en 33170383205 (2× 88 % < 90 %). **Diagnose 28-08:** chunks staan in de index (BM25 22, categorie `werk_inkomen`), maar de classifier kiest domein `welzijn`; het categoriefilter levert alleen welzijn-subsidies met RRF ≈ 0,03 > `EscalationScoreThreshold` 0,015, dus de retry zonder categorie vuurt niet en de generatie ziet de ZoetermeerPas nooit. Opties: (a) retry zonder filter óók wanneer de generatie `no_cited_answer` geeft, (b) gefilterde en ongefilterde hits samenvoegen (RRF over beide), (c) `werk_inkomen` als tweede categorie bij `welzijn`-intent. Trace: c9891277c1314d81aa24337afc2d1c03 | eval-stabiliteit | `AskOrchestrator.cs:65-81`, `PolicyVersion.cs`, `eval/cases.yaml` | ✅ gedaan (29-08, plan 5 / PR-URL): gekozen is een vierde optie — meervoudige categorieën per fragment plus de categorie als tag-boost i.p.v. filter, waarmee de retry-tak verviel ([ADR-0006](../../adr/0006-categorie-als-boost-niet-als-filter.md)) |
