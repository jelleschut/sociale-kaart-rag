...
public class OsmOverpassSourceTests
{
    ...
    {
        var (records, _) = OsmOverpassSource.ParseWithReport(Fixture());
        ...
        Assert.Contains(records, r => r.Municipality == "Zoetermeer");
        Assert.Contains(records, r => r.Municipality == "Den Haag");
    }
    ...
    public void Elements_without_name_or_category_are_skipped_and_counted()
    {
        ...
        var (records, skipped) = OsmOverpassSource.ParseWithReport(json);
    ...
    [Fact]
    public void Query_covers_both_municipalities_without_office()
    {
        ...
        Assert.DoesNotContain("office", OsmOverpassSource.Query);
    }
}
